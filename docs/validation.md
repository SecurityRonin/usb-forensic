...
| DriverFrameworks-UserMode event log | *none available* — the log is disabled by default on Win8+, so no public forensic corpus (Szechuan, DFIRArtifactMuseum, EVTX-ATTACK-SAMPLES, hayabusa) ships it with ground truth | **Two independent authoritative field maps**: EvtxECmd (Eric Zimmerman / Hyun Yi) + rvt2 (IncideDigital) | EID 2003 (`UMDFHostDeviceArrivalBegin`) → `LastConnected`, EID 2102 (`UMDFHostDeviceRequest`) → `LastRemoved`, keyed by instance serial; instance read from the `UserData` child's `instance` attribute with `InstanceId`-element fallback (both map forms). Non-false-positive control: runs clean on the real Kernel-PnP `.evtx` (a non-DriverFrameworks log → zero DriverFramework claims). Env-gated real-`.evtx` test ready for the Tier-1 backstop | 3 (spec) |
